...
for package in libpcap snapd bison gcc make curl; do
    if ! equery list $package; then
        echo $SUDO_PASSWORD | sudo -S emerge --ask $package
    fi
done

if ! command -v snap &> /dev/null; then
    echo "Snap is not installed. Installing now..."
    echo $SUDO_PASSWORD | sudo -S emerge --update --newuse @world
    echo $SUDO_PASSWORD | sudo -S emerge --ask snapd
    if command -v snap &> /dev/null; then
